...
function FaqsSearch(id){
	...
	if(!$('#fqas_keyword').val()){
		faqs_title='';
		myMessage.add('入力してください', 'warning');
	}
	if(!toloading){
		toloading = true
		$('#upLoading').text('加载中...')
		api.GET('/content/faqs/page',{
            ...
            if(res.code===200){
                ...
                if(res.data.content.length==0){
                    myMessage.add("検索結果なし、別の単語を試してください", 'error');
                    return;
                }else {
                    if($("body").width() > 768){
                        $(".faqs_btm_right ").hide();
                    ...
                    if($("body").width() > 768){
                        for(var i=0;i<data.length;i++){
                            if(data[i].contentTxts.length>0){
                                var faqs_result = '<li id="index_'+ data[i].channelName +'" class="faqs_search"><p class=""><a href="javascript:;"><span class="fqas-icon"></span>Q:'+ data[i].title +'</a></p> </li><div class="faqs_btm_right_content hide">'+ data[i].contentTxts[0].attrTxt +'</div>'
                        ...
                        for (var i = 0; i < data.length; i++) {
                            var faqs_result = '<li><p class=""> <a href="javascript:;"><span class="fqas-icon"></span>Q:' + data[i].title + '</a></p></li><div class="faqs_btm_right_content hide">' + data[i].contentTxts[0].attrTxt + '</div>'
                            html.push(faqs_result);
...
function loading(){
	if(!last && !toloading){
		...
		myMessage.add('これ以上のデータはありません', 'warning');
	}
}
