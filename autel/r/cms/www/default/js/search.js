...
function search(){
    if(!last){
        ...
        if($("body").width() > 769){
            parms.keyword = $('#keyword').val().toLowerCase();
        }else {
            parms.keyword = $('#kw_751').val().toLowerCase();
        }
        api.GET('/content/search',parms,function(data){
            if(data.content.length>0){
                ...
                if(last){
                   ...
                   $('.loading').css('background-color','#fff');
                    ...
                    $('.loading').text('VIEW MORE')
                }
                $('.loading').show();

                for(var i=0;i<listData.length;i++){
                    var searchRow = $('<div class="search-item overflow"><div class="item-right fl"><a href="'+listData[i].url+'" class="Regular keywords search_title">'+listData[i].title+'</a><div class="addone none">+1</div><p class="search-dcs">'+listData[i].description+'</p></div></div>')
